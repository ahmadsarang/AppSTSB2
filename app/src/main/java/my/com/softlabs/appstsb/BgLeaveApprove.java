...
public class BgLeaveApprove extends AsyncTask<String, Void, String> {

    String approveLeave_url;

    AlertDialog alertDialog;
    Context ctx;
    BgLeaveApprove(Context ctx)
    {
        this.ctx =ctx;
    }
    @Override
    ...
    protected String doInBackground(String... params) {
        ...
        approveLeave_url = Config.URL_LEAVE_APPROVE + dept;
        ...
        if (method.equals("apply")) {
            ...
            String manRem = params[3];
            String leave_id = params[5];
            String leave_bal = params[7];
            String tot_apply = params[8];
            String leave_bal_after = params[9];
            try {
                URL url = new URL(approveLeave_url);
                HttpURLConnection httpURLConnection = (HttpURLConnection) url.openConnection();
                httpURLConnection.setRequestMethod("POST");
                ...
                String data =
                        URLEncoder.encode("man_name", "UTF-8") + "=" + URLEncoder.encode(name, "UTF-8") + "&" +
                                ...
                                URLEncoder.encode("manager_remark", "UTF-8") + "=" + URLEncoder.encode(manRem, "UTF-8") + "&" +
                                URLEncoder.encode("leave_id", "UTF-8") + "=" + URLEncoder.encode(leave_id, "UTF-8") + "&" +
                                URLEncoder.encode("leave_bal", "UTF-8") + "=" + URLEncoder.encode(leave_bal, "UTF-8") + "&" +
                                URLEncoder.encode("tot_apply", "UTF-8") + "=" + URLEncoder.encode(tot_apply, "UTF-8") + "&" +
                                URLEncoder.encode("leave_bal_after", "UTF-8") + "=" + URLEncoder.encode(leave_bal_after, "UTF-8");
                bufferedWriter.write(data);
                bufferedWriter.flush();
                bufferedWriter.close();
                OS.close();
